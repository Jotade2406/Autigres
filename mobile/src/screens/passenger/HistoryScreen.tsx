...
import type { TripHistoryItemDto } from '../../api/types';

// ── Date grouping helpers ─────────────────────────────────────────────────────

function parseUtc(iso: string): Date {
  // Backend sends UTC without 'Z' — append it so JS converts to local time correctly
...
function sectionTitle(iso: string): string {
  const d    = parseUtc(iso);
  const now  = new Date();
  const todK = dayKey(now.toISOString());
  const k    = dayKey(iso);
  if (k === todK) return 'Hoy';
...
function tripTime(iso: string): string {
  return parseUtc(iso).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit', hour12: false });
}

interface Section { title: string; data: TripHistoryItemDto[] }

function buildSections(items: TripHistoryItemDto[]): Section[] {
  const map = new Map<string, { title: string; data: TripHistoryItemDto[] }>();
...
function TripCard({ item }: { item: TripHistoryItemDto }) {
  const cancelled = item.status === 'cancelled';
  const fare      = cancelled ? 'Bs. 0' : `Bs. ${Math.round(item.fareAmount)}`;
  ...
  return (
    <View style={S.card}>
      ...
      <View style={S.cardCenter}>
        ...
        {item.sharedWith ? (
          <Text style={S.sharedWithText} numberOfLines={1}>Con {item.sharedWith}</Text>
        ) : null}
      </View>
  ...
  return (
    <SafeAreaView style={S.safe}>
      ...
      <SectionList
        ...
        ListHeaderComponent={<Text style={S.screenTitle}>Historial</Text>}
        ListEmptyComponent={
...
const S = StyleSheet.create({
  ...
  sharedWithText: { fontSize: 11, fontWeight: '600', color: '#7C6FFF' },
